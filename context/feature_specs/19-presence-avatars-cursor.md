...
1. Keep the existing navbar behavior as-is.
    - do not change the editor home avatar
    - do not move or redesign the shared navbar component globally
    - if the editor home and editor canvas use the same navbar component, 
    make sure this presence UI only appears in the canvas/editor room view
...
2. Add the participant avatar group inside the editor canvas area.
    ...
    - show a divider between the collaborator avatars and the Clerk UserButton only when at least one collaorator exists
...
3. Render collaborator avatars.
    ...
    - fall back to initials where there is no image
    - show up the five collaborator avatars in an overlapping stack
...
- Divider only appears when collaborator exist.
