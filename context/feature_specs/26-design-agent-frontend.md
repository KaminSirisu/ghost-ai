Wire up the AI sidebar so users can submit design prompt, track AI run status in real time, and reflect AI-driven canvas updates through Liveblocks.
...
4. Status display
- Read the latest message from `ai-statis-feed`
...
## Check When Done
- Submitting a prompts calls `/api/ai/design` and returns a `runId`
...
- Chat update appear across multiple sessions
