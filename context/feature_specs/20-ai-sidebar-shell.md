...
4. Build the AI Architect tab.
    ...
    Use shadcn components where they fit, espeacially `Button` and `Textarea`.
...
5. Build the Specs tab.
    ...
    - include a file/icon, title, short snippet, and disabled download action

6. Use the existing project color tokens.

    Check `global.css`, `ui-context.md` or the tailwind mapping before adding direct color values. Avoid inventing new colors if a matching token already exists.
