...
1. Add connection handles to every node.
    ...
    - users should be able to connect from any handle to any othe handle 
...
3. Creat the custom edge renderer.
    - use clean right-angle routing
    - keep edges slightly dimmed at rest
    - brighten edges when hovered or selected
    - make edeges easier to hover and click without increasing the visible line thickness
...
- keep this focused on edge rendering, labels, and connection beahvior
...
- Edge label position use `EdgeLabelRenderer` and path midpoint coordinates.
