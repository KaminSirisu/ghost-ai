...
Create or update `trigger/generate-spec.ts`. Define a `generateSpec` task that:
    - accepts `projectId`, `roomId`, `chatHistory`, `nodes`, and `edges`
    - validates input with Zod
    - uses Gemini through `@ai-sdk/google`
    - generates a Markdown technical spec from the canvas and chat control
...
- `POST /api/ai/spec` validates input and return a `runId`
